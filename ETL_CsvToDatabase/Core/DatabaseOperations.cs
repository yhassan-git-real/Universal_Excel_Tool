...
namespace ETL_CsvToDatabase.Core
{
    public static class DatabaseOperations
    {
            ...
            string fileName)
        {
            ...
            ConsoleLogger.LogInfo("validation", $"Temp table columns ({tempColumns.Count}): {string.Join(", ", tempColumns)}");
            ConsoleLogger.LogInfo("validation", $"Dest table columns ({destColumns.Count}): {string.Join(", ", destColumns)}");
    ...
    ValidationResult validationResult)
        {
            try
            {
                ...
                long tempTableCount = await GetTableRowCountAsync(connection, tempTableName);
                long initialDestCount = await GetTableRowCountAsync(connection, destinationTableName);
                
                ConsoleLogger.LogInfo("transfer", $"Source (Temp) Table Rows: {tempTableCount:N0}");
                ConsoleLogger.LogInfo("transfer", $"Destination Table Rows (Before): {initialDestCount:N0}");
                ConsoleLogger.LogInfo("transfer", $"Columns to Transfer: {validationResult.MatchedColumns.Count}");
            ...
            FROM [{tempTableName}]";
                ...
                long finalDestCount = await GetTableRowCountAsync(connection, destinationTableName);
                long rowsTransferred = finalDestCount - initialDestCount;
                
                ConsoleLogger.LogInfo("transfer", $"Destination Table Rows (After): {finalDestCount:N0}");
